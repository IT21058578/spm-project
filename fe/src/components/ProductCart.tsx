...
const ProductCart = (props: ProductType) => {
    ...
    const filteredReviews = reviews?.content.filter((review: Review) => review.productId === props._id) || [];
        ...
        props.type === 'list' ?
            ...
            <div className="position-relative bg-white mt-4 border-1 border fd-hover-border-primary product-cart" style={{ minHeight: "250px" }}>
                ...
                <div className="d-flex gap-3">
                    <div className="position-relative h-100 col-3 product-img">
                        <img src={props.images[1]} alt={props.type} className="w-100 h-50" />
                        {/* <img src={props.img} alt={props.name} className="w-100 h-50" /> */}
                    </div>
                    <div className="w-75 pt-3">
                        <div>
                            <Link to={"/product/" + props._id} className="product-name my-2 fw-bold text-dark">{props.name}</Link>
                        </div>
                        <div className='d-flex my-2'>
                            <Reviews rating={averageRating} />
                        </div>
                        <div className="d-flex">
                            <h5 className="fd-color-primary">RS {props.price}</h5>
                            <h6 className="align-self-end" style={{ textDecoration: "line-through" }}>RS {props.price - 100}</h6>
                        </div>
                        <p className="desc fw-bold opacity-50">{props.brand}</p>
                        <div className="d-flex gap-2">
                            <AddToCart product={props}/>
                            <div><Link to={"/product/" + props._id} className="fd-btn rounded-3"><i className="bi bi-eye"></i></Link></div>
            ...
            <div className="position-relative bg-white mt-4 border-1 border fd-hover-border-primary product-cart" style={{ height: "400px" }}>
                <div className="position-relative h-75 product-img">
                    <img src={props.images[1]} alt={props.type} className="w-100 h-100" />
                ...
                <Link to={"/product/" + props._id} className="show position-absolute fd-btn rounded-5 d-none animate__animated animate__fadeInRight" style={{ padding: '5px 10px', top: '20%', right: '10%' }}> <i className="bi bi-eye"></i></Link>
                ...
                <div className="d-flex justify-content-center">
                    <h5 className="fd-color-primary">RS {props.price}</h5>
                    <h6 className="align-self-end" style={{ textDecoration: "line-through" }}>RS {props.price}</h6>
                </div>
            </div>
    )
