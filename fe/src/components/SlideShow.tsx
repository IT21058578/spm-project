...
import { apiSlidesInfo } from "../views/VirtualData";
...
const SlideShow = (props: { options?: {} }) => {
  ...
  return (
    ...
    //   {!isLoading && !isError ?
    //     apiSlidesInfo.map((slide: Slide) => {
    //       return <div className='f-slide position-relative shadow h-100' key={slide.alt}>
    //         <img src={slide.url} alt={slide.text} className="w-100 h-100" />
    ...
    <Carousel
      ...
      interval={3000}
    >
      {apiSlidesInfo.map((slide, index) => (
        <div className="f-slide position-relative shadow h-100" key={index}>
